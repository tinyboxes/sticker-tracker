...
export default function StickerTracker() {
  ...
  const tradeList = useMemo(() => {
    ...
    for (const s of list) {
      ...
      // so we back those out — except Blue, which is common enough that having
      // a Blue parallel doesn't reserve the base copy too. Rarer colors
      // (Red/Purple/Orange/Green/Black) still each reserve one base copy.
      const parallels = s.parallels || {};
      const reservingParallelTotal = Object.entries(parallels).reduce(
        (sum, [color, count]) => (color === "Blue" ? sum : sum + (count || 0)),
        0
      );
      const tradeableBase = s.owned - 1 - reservingParallelTotal;
      if (tradeableBase > 0) {
  ...
  function loadOfficialChecklist() {
    setStickers((prev) => {
      const next = { ...prev };
      for (const [number, team, player, foil] of [...CHECKLIST_2026_NA, ...UPDATE_SET_2026]) {
        if (next[number]) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#141210", fontFamily: "'Inter', sans-serif", color: "#F4EFE1", paddingBottom: 90 }}>
      ...
      <div className="pitch-stripe" style={{ padding: "16px 16px 12px", position: "sticky", top: 0, background: "#141210", zIndex: 10, borderBottom: "1px solid rgba(244,239,225,0.12)" }}>
        ...
        {/* Search — always right here, never buried */}
          ...
          {searchFocused && query.trim() && suggestions.length > 0 && (
            <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0, background: "#211D17", border: "1px solid rgba(244,239,225,0.2)", borderRadius: 8, overflow: "hidden", zIndex: 30, boxShadow: "0 6px 16px rgba(0,0,0,0.4)" }}>
              {suggestions.map((s) => (
                <button
                  key={s.number}
                  onClick={() => { setQuery(s.number); setSearchFocused(false); }}
        ...
        {/* Progress + parallels count — hidden on Trade tab, not relevant while trading */}
        {tab !== "trade" && (
          <>
            ...
            {totals.parallels > 0 && (
              <div style={{ fontSize: 11, opacity: 0.7, marginTop: 6 }}>✦ {totals.parallels} parallel{totals.parallels !== 1 ? "s" : ""} logged</div>
            )}
          </>
        )}
      </div>
